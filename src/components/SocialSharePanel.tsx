...
const SOCIAL_LINKS = [
  ...
  },
  {
    ...
    icon: (
      <svg aria-hidden="true" width={18} height={18} fill="currentColor" viewBox="0 0 24 24"><path d="M6.94 6.577a2.117 2.117 0 11-.001-4.235 2.117 2.117 0 010 4.235zm.03 2.254c-1.767 0-3.1 1.174-3.1 3.27V20h3.328v-7.69c0-.595.07-1.153.425-1.525.35-.37.895-.516 1.418-.516 1.144 0 1.463.737 1.463 1.814V20h3.328v-8.16c0-2.73-1.457-4.003-3.333-4.003-1.23 0-1.839.692-2.19 1.178v.002zm14.03.468h-3.298V20h3.328v-5.372c0-1.322.25-2.632 1.972-2.632 1.703 0 1.72 1.531 1.72 2.707V20H24v-6.331c0-3.076-1.634-4.507-3.806-4.507-1.56 0-2.25.778-2.626 1.326h.002v-1.12z"/></svg>
    ),
...
export const SocialSharePanel = () => {
  ...
  return (
    <Card className="w-full max-w-md mx-auto shadow-md border-0 bg-white/90 dark:bg-gray-800/70 backdrop-blur-sm p-0">
      <CardHeader className="py-3 px-4">
        <CardTitle className="flex flex-col items-center text-base font-semibold text-gray-800 dark:text-gray-100 gap-1">
          <UserPlus2 className="h-5 w-5 text-indigo-500 mb-1" />
        ...
        <p className="text-xs text-gray-600 dark:text-gray-300 mt-2 text-center font-medium">
          ...
          <span className="text-xs text-blue-700 dark:text-blue-300 font-normal">(You’ll be listed on our early supporters leaderboard soon! 🚀)</span>
        </p>
      </CardHeader>
      <CardContent className="p-0 pt-2 pb-4 flex flex-col items-center">
        <div className="flex flex-col flex-wrap gap-2 sm:flex-row sm:justify-center w-full items-center px-3">
          {SOCIAL_LINKS.map((item) => (
            ...
            >
              <Button
                variant="default"
                className={`w-full min-w-[120px] justify-center ${item.color} font-semibold gap-2 shadow hover:scale-105 transition text-sm py-1`}
          ...
          <Button
            variant="outline"
            className="w-full sm:w-auto font-semibold gap-2 text-sm py-1 min-w-[120px] justify-center"
            onClick={handleCopy}
